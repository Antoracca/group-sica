...
import { UNIT_LABEL } from "@/lib/devis/catalog";
import {
  formatFcfa,
  formatDateFr,
  lineQty,
...
function Field({
  ...
  return (
    <div className={wide ? "col-span-2" : ""}>
      <span className="block font-mono text-[0.52rem] uppercase tracking-[0.12em] text-slate">
        {label}
      </span>
      {filled ? (
        <span className="mt-0.5 block text-[0.82rem] font-medium leading-snug text-ink">
          {value}
        </span>
      ) : (
...
function SectionTitle({ index, children }: { index: string; children: ReactNode }) {
  return (
    <div className="mb-3 flex items-center gap-2.5">
      <span className="font-mono text-[0.6rem] font-bold text-brand-amber">{index}</span>
      <h2 className="font-display text-[0.95rem] font-semibold uppercase tracking-[0.08em] text-brand-royal">
        {children}
      </h2>
...
const COLS = "grid grid-cols-[1fr_3.5rem_3.5rem_6rem_7rem] gap-2";

function TableHead({ continued }: { continued: boolean }) {
  return (
    <div
      className={`${COLS} border-b-2 border-brand-royal/80 pb-1.5 font-mono text-[0.55rem] font-semibold uppercase tracking-[0.1em] text-brand-royal`}
    >
      <span>
...
function Row({
  ...
  return (
    <div className={`${COLS} items-center border-b border-black/5 py-2 text-[0.74rem] text-ink`}>
      <span>
        <span className="font-medium">{label}</span>
        {detail ? (
          <span className="mt-0.5 block text-[0.6rem] leading-snug text-slate">{detail}</span>
        ) : null}
      </span>
    ...
    case "intro":
      return (
        <div key={key} className="mb-5">
          ...
          <p className="mt-3 text-[0.74rem] leading-relaxed text-slate">
    ...
    case "totals":
      return (
        <section key={key} className="mt-5 flex justify-end">
          <div className="w-[20rem] text-[0.78rem]">
    ...
    case "conditions":
      return (
        <section key={key} className="mt-5">
          ...
          <div className="grid grid-cols-2 gap-6 text-[0.7rem] leading-relaxed text-slate">
            <div>
    ...
    case "signature": {
      ...
      return (
        <section key={key} className="mt-6">
          <p className="mb-3 text-[0.7rem] text-slate">
...
function SignatureBox({
  ...
  return (
    <div>
      <p className="font-mono text-[0.58rem] uppercase tracking-[0.12em] text-brand-royal">
        {title}
      </p>
      <p className="text-[0.62rem] text-slate">{subtitle}</p>
      <div className="relative mt-2 flex h-24 items-center justify-center rounded-md border border-dashed border-slate/40 bg-mist/40">
        {image ? (
          ...
          <span className="font-mono text-[0.55rem] uppercase tracking-[0.14em] text-slate/60">
            Signature
          </span>
        ) : null}
